...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # 0. Retrieve element of self.data
        document = self.data[idx]

        # 1. Randomly truncate document
        rand_length = random.randint(4, int(self.block_size * 7/8))
        document = document[:rand_length]

        # 2. Break document into substrings: [prefix][masked_content][suffix]
        prefix_length = random.randint(1, rand_length - 2)
        masked_content_length = random.randint(1, int(2*rand_length/4))
        ...
        # 3. Rearrage substrings into: [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]
        temp_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        masked_string = temp_string + self.PAD_CHAR*(self.block_size - len(temp_string))
        
        # 4. Construct input and output example pair using masked_string
        x = masked_string[:-1]
        y = masked_string[1:]

        # 5. Using vocabulary, encode resulting input and output strings and return data point
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
        return x, y
