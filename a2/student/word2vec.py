...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    y_hat = softmax(np.dot(outsideVectors, centerWordVec)) # P(O|C=c)
    y = np.zeros(np.shape(y_hat)) # create one-hot vector
    y[outsideWordIdx] = 1

    loss = -np.log(y_hat[outsideWordIdx]) # -log(P(O=o|C=c))
    gradCenterVec = np.dot(outsideVectors.T, y_hat-y) # v_c(y_hat - y) 
    gradOutsideVecs = np.outer((y_hat-y).T, centerWordVec)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    u_wk = outsideVectors[negSampleWordIndices]
    v_c = centerWordVec

    ### Please use your implementation of sigmoid in here.
    loss = -np.log(sigmoid(np.dot(u_o.T, v_c))) - np.sum(np.log(sigmoid(np.dot(-u_wk, v_c))))

    gradCenterVec = -(1 - sigmoid(np.dot(u_o.T, v_c))) * u_o + np.sum((1 - sigmoid(np.dot(-u_wk, v_c)))[:, np.newaxis] * u_wk, axis=0)
    
    gradOutsideVecs = np.zeros(outsideVectors.shape)

    gradOutsideVecs[outsideWordIdx, :] = -(1 - sigmoid(np.dot(u_o.T, v_c))) * v_c # calculate for partial wrt u_o
    unique, ind, counts = np.unique(negSampleWordIndices, return_index=True, return_counts=True) # unique elements array, array of indices of those words, array of number of occurrences
    uniqueSamples = sigmoid(-np.dot(u_wk, v_c))[ind]
    gradOutsideVecs[unique, :] = (-(1 - uniqueSamples) * counts)[:, np.newaxis] * -v_c # calculate for partial wrt u_w_s
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        tmpLoss, tmpGradCenterVec, tmpGradOutsideVecs = word2vecLossAndGradient(centerWordVec, word2Ind[word], outsideVectors, dataset) 
        loss += tmpLoss
        gradCenterVecs[centerIdx] += tmpGradCenterVec
        gradOutsideVectors += tmpGradOutsideVecs
    ### END YOUR CODE
